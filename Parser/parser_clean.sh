...
# Use the NEW darshan-parser from fresh installation
export PATH="$HOME/darshan-fresh/bin:$PATH"

# Activate conda environment
if [ -f "$HOME/miniconda3/etc/profile.d/conda.sh" ]; then
  source "$HOME/miniconda3/etc/profile.d/conda.sh"
  conda activate gnn4_env || conda activate ior_env || true
fi
...
echo "Using darshan-parser from: $(which darshan-parser)"
echo "=========================================="
...
# Run the parser with the correct Python path
/u/mbanisharifdehkordi/.conda/envs/gnn4_env/bin/python Parser/parser_clean.py  "$INPUT_DIR" "$OUTPUT_CSV" "$SAMPLE_CSV" "$TEMP_DIR"
PARSER_EXIT=$?
